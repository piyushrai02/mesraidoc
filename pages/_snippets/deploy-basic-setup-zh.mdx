...
<AccordionGroup>
  <Accordion title="推荐硬件">
    按默认 `local` 沙箱模式（mesrai-graph + cross-file context 在 worker
    容器内运行）调整大小。
  ...
  <Accordion title="服务">
    ...
    **核心（始终运行）：**
    ...
    - `mesrai-web` — Next.js 前端
    - `db_mesrai_postgres`、`db_mesrai_mongodb`、`rabbitmq` —
      本地基础设施。使用 `USE_LOCAL_DB=false` /
      `USE_LOCAL_RABBITMQ=false` 跳过以指向托管实例。

    **可选：**
    - `mesrai-mcp-manager` — Model Context Protocol broker。通过
      `API_MCP_SERVER_ENABLED=true` 启用。请参阅
      [MCP Manager](/how_to_deploy/zh/deploy_mesrai/mcp_manager)。
    - `worker-analytics` — Cockpit 摄取（DORA 指标、PR classifier）。
      **仅限自托管企业版**，默认不连接。请参阅
      [Analytics Worker](/how_to_deploy/zh/deploy_mesrai/analytics_worker)。
  </Accordion>
</AccordionGroup>
...
<Note>
  ...
  [Sandbox & AST Graph](/how_to_deploy/zh/deploy_mesrai/sandbox)。
</Note>
...
- 使用独立的 webhook 子域名，例如 `mesrai-webhooks.yourdomain.com`，或
...
<Steps>
  ...
  <Step title="Edit the environment file">
    ...
    详细说明请参见 [环境变量配置](/how_to_deploy/zh/deploy_mesrai/generic_vm)。  
  </Step>
