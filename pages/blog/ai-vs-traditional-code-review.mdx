...
AI handles the heavy lifting with automated checks:
- Security scanning
...
Configure thresholds via the [Mesrai Dashboard](https://app.mesrai.com):
...
Here's how to implement hybrid review via the [Mesrai Dashboard](https://app.mesrai.com):

**AI Review Settings** (Settings → Review Rules):
- Enable automatic reviews
- Set blocking issues: security critical, performance regression, test coverage drop
- Configure auto-approve criteria: score >= 90, no warnings, PR size < 200 lines

**Human Review Requirements** (Settings → Review Workflow):
- Required for: architectural changes, API changes, database migrations, critical paths, large PRs (500+ lines)
- Optional for: minor fixes, documentation, test updates, style changes
...
Document when each type applies via dashboard settings:
...
- 🔒 Zero code storage, sandbox processing, TLS 1.3 encryption
