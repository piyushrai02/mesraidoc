...
description: "Configure Mesrai Playbook and Memories to enforce audit trail, data handling, and access control requirements."
icon: "file-shield"
---
...
Start by teaching Mesrai the high-level compliance principles:

```
@mesrai remember: we are SOC2 compliant. All data access must be logged
with who accessed what, when, and from where.
```

```
@mesrai remember: PII (personally identifiable information) must never
appear in logs, error messages, or API responses. PII includes:
name, email, phone, address, SSN, date of birth, IP address.
```

```
@mesrai remember: all data at rest must be encrypted. Database fields
containing PII must use application-level encryption.
```

```
@mesrai remember: access control follows least privilege principle.
