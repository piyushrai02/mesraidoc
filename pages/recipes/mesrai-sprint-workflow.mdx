...
Once connected, Mesrai can automatically fetch task context during reviews.
...
The quality of validation depends entirely on the quality of your task descriptions. Mesrai classifies task context as:
...
```
@mesrai remember: every PR must be linked to a Jira ticket.
PRs without a linked ticket should be flagged.
```

```
@mesrai remember: acceptance criteria marked as "out of scope"
...
1. **Code quality review** — security, performance, style (standard Mesrai)
...
- If a PR intentionally doesn't cover all criteria (phased delivery), note it in the PR description so Mesrai can account for it
