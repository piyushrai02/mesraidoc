...
description: "Set up Mesrai Playbook focused on OWASP top 10, secrets detection, and secure coding patterns."
icon: "shield-halved"
---
...
This gives you Mesrai' built-in security checks. The rules below add your team's specific security policies on top.
...
```
@mesrai remember: all user input must be validated and sanitized
before use. Never trust client-side data.
```

```
@mesrai remember: all API endpoints that handle sensitive data
must use HTTPS only and include rate limiting.
```

```
@mesrai remember: error messages must never expose stack traces,
...
1. Enable **Request Changes** in PR Workflow settings so Mesrai requests changes when critical issues are found
...
For more on configuring rules, see [Mesrai Playbook](/guides/code_review/configs/mesrai_rules).
