...
- **Mesrai Playbook** (file-level and PR-level)
- **Review options** (which analysis types to enable)
- **Noise control** (severity filters, max suggestions)
- **Ignored paths**
...
```
@mesrai remember: shared libs must maintain backward compatibility.
...
For more on directory configs, see [Directory-Level Configuration](/guides/code_review/configs/directory_level). For rule inheritance, see [Rules Inheritance](/guides/code_review/configs/rules_inheritance).
