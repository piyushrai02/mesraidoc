...
description: "Create custom Mesrai Playbook directly in your repository using markdown files with structured templates."
---
...
<Info>
Repository Rules use the same auto-sync mechanism as IDE rule file detection. Enable "Auto-sync rules from repo" in settings to activate both features. See <a href="/guides/code_review/configs/rules_file_detection">Rules File Detection</a> for setup.
</Info>

## How to Use
You can create custom Mesrai Playbook directly in your repository by placing structured markdown files in specific directories. This allows you to version control your rules alongside your code and share them across your team.

### Synchronization
- **Automatic Detection**: Repository rules are automatically detected and synchronized when enabled
- **Manual Sync**: Add `@mesrai-sync` to any rule file to sync it individually (works even with auto-sync disabled)
- **Web Application**: Synchronized rules appear in your Mesrai web application dashboard
...
2. **Create rule files**: Place `.md` files in `.mesrai/rules/**` or `rules/**` directories
3. **Automatic sync**: All rule files sync when PRs are closed

### Option 2: Manual Sync (Selective)
1. **Create rule files**: Place `.md` files in `.mesrai/rules/**` or `rules/**` directories
2. **Add sync marker**: Include `@mesrai-sync` anywhere in the rule file
3. **Commit changes**: Only marked files will sync (auto-sync toggle stays off)

<Tip>
Repository Rules use the same synchronization mechanism as [Rules File Detection](/guides/code_review/configs/rules_file_detection). Use manual sync (`@mesrai-sync`) to selectively sync specific rules without enabling automatic synchronization.
</Tip>