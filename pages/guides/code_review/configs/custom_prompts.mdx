...
Custom Prompts give you complete control over how Mesrai analyzes your code and communicates findings. Tailor what the AI looks for in each review category, how it classifies severity, and how it presents suggestions to match your team's standards and communication style.
...
the company's naming convention. According to the Mesrai Rules, all new files 
must use camelCase. Please rename the file to 'createOrUpdateIssuesParameter.dto.ts' 
to adhere to the standard.
```

Your custom prompt instructs Mesrai on how to write suggestion messages. The prompt receives context about the severity level, problem description, suggested code, and whether it's a Mesrai Rules violation. Mesrai uses this context along with your prompt to generate appropriate messages for each suggestion.
...
All custom prompts—Category, Severity, Suggestion, and PR Summary—support referencing real files from the codebases Mesrai can access. When you save an update, Mesrai automatically detects external references and resolves them for future reviews.
...
<Tabs>
  <Tab title="Good: Meaningful Difference">
    ```
    For Mesrai Rules violations, use a stricter tone: 
  ...
  <Tab title="Bad: Superficial Label">
    ```
    For Mesrai Rules, add "[RULE]" prefix before the problem.
...
<AccordionGroup>
  ...
  <Accordion title="Different Tone for Rules vs Suggestions">
    **Scenario:** Mesrai Rules are strict requirements, suggestions are recommendations
    
    **Custom Suggestion Prompt:**
    ```
    Use different tones based on the type:
    
    For Mesrai Rules violations:
...
<AccordionGroup>
  ...
  <Accordion title="Can I customize prompts with mesrai-config.yml?">
    No. Custom prompts are web-only to ensure intentional changes. Other settings (ignored paths, branches) can use `mesrai-config.yml`.
  </Accordion>
  ...
  <Accordion title="Do suggestion prompts work for both Mesrai Rules and standard suggestions?">
