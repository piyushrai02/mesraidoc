---
title: "Prompt Studio"
description: "Customize Mesrai's review prompts for each category and severity level to match your team's needs"
---
...
Prompt Studio give you complete control over how Mesrai analyzes your code and communicates findings. Tailor what the AI looks for in each review category, how it classifies severity, and how it presents suggestions to match your team's standards and communication style.

<Card
  title="Access Prompt Studio Settings"
  icon="gear"
  href="https://app.mesrai.com/settings/code-review/global/custom-prompts"
>
...
<CardGroup cols={2}>
  <Card title="Team-Specific Standards" icon="users">
    Align Mesrai's reviews with your team's unique coding standards, architectural patterns, and best practices
  </Card>
  ...
  <Card title="Focus Control" icon="bullseye">
    Direct Mesrai's attention to issues that matter most to your project and reduce noise
  </Card>
  <Card title="Communication Style" icon="comments">
    Customize how Mesrai presents suggestions - detailed for critical issues or concise for minor ones
  </Card>
</CardGroup>

## How Prompt Studio Work

Prompt Studio operate at three levels:

### Category Prompts

Define what Mesrai looks for in each review category:
...
Define how Mesrai classifies issue severity:
...
Control how Mesrai communicates each individual suggestion. This customizes the message format and tone that appears in PR comments, like:

```
The new file 'create-or-update-issues-parameter.dto.ts' does not follow 
the company's naming convention. According to the Mesrai Playbook, all new files 
must use camelCase. Please rename the file to 'createOrUpdateIssuesParameter.dto.ts' 
to adhere to the standard.
```

Your custom prompt instructs Mesrai on how to write suggestion messages. The prompt receives context about the severity level, problem description, suggested code, and whether it's a Mesrai Playbook violation. Mesrai uses this context along with your prompt to generate appropriate messages for each suggestion.
...
All custom prompts—Category, Severity, Suggestion, and Summary Style—support referencing real files from the codebases Mesrai can access. When you save an update, Mesrai automatically detects external references and resolves them for future reviews.

- Use `@file:path/to/file.ts` to point at files in the repository you're editing; Mesrai will try to locate the path locally first.
...
- After saving, Mesrai processes the references in the background. Check the status indicator beside the prompt editor to confirm when resolution completes.
...
<Tabs>
  <Tab title="Good: Meaningful Difference">
    ```
    For Mesrai Playbook violations, use a stricter tone: 
  ...
  <Tab title="Bad: Superficial Label">
    ```
    For Mesrai Playbook, add "[RULE]" prefix before the problem.
...
<AccordionGroup>
  <Accordion title="Industry-Specific Requirements">
    ...
    **Benefit:** Mesrai catches compliance-specific issues that generic security reviews might miss.
  </Accordion>
  ...
  <Accordion title="Different Tone for Rules vs Suggestions">
    **Scenario:** Mesrai Playbook are strict requirements, suggestions are recommendations
    
    **Custom Suggestion Prompt:**
    ```
    Use different tones based on the type:
    
    For Mesrai Playbook violations:
...
<AccordionGroup>
  <Accordion title="Reviews Are Too Noisy">
    ...
    **Solutions:**
    ...
    - Raise minimum severity threshold in [Suggestion Control](/guides/code_review/configs/suggestion_control)
    - Check for redundancy between category and severity prompts
  </Accordion>

  <Accordion title="Missing Expected Issues">
    **Symptoms:** Mesrai doesn't catch issues it should
...
<AccordionGroup>
  ...
  <Accordion title="What happens to existing PRs when I change prompts?">
    Changes only affect new reviews. Re-run with `@mesrai start-review` to apply new prompts to existing PRs.
  </Accordion>

  <Accordion title="Can I customize prompts with mesrai-config.yml?">
    No. Prompt studio are web-only to ensure intentional changes. Other settings (ignored paths, branches) can use `mesrai-config.yml`.
  </Accordion>
  ...
  <Accordion title="What's the difference between Category/Severity prompts and Suggestion prompts?">
    **Category/Severity prompts** control what Mesrai analyzes and how it classifies issues. **Suggestion prompts** control how Mesrai presents findings to your team. You can customize both independently.
  </Accordion>
  ...
  <Accordion title="What happens if my suggestion template has syntax errors?">
    Mesrai will fall back to the default suggestion format and notify you of the error in settings.
  </Accordion>

  <Accordion title="Do suggestion prompts work for both Mesrai Playbook and standard suggestions?">
    Yes. Use the `isMesraiRule` variable to create different messages for rules vs suggestions in the same prompt.
  </Accordion>
</AccordionGroup>
