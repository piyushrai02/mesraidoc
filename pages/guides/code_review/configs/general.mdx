---
title: "General"
description: "General review configuration — languages, severity thresholds, output format."
---
...
<Note>
  Mesrai has three config levels: Global, Repository, and Directory. Learn how they work in <a href="/guides/code_review/configs/inheritance_overrides">Config Inheritance & Overrides</a>.
</Note>

### Config Priority
`mesrai-config.yml` in your repo root overrides web settings. No setup needed—just commit the file.

<Accordion title="Sample mesrai-config.yml">
```yaml
version: '1.2'
summary:
...
Mesrai can review PRs automatically when opened, or only when you comment `@mesrai start-review`.
...
How Mesrai handles reviews on subsequent commits:
...
<Tabs>
  ...
  <Tab title="Manual">
    Only run follow-up reviews when you comment `@mesrai start-review`.
  </Tab>
</Tabs>
...
#### Auto-approve PRs
Mesrai approves PRs when no issues are found.

**Platforms:** GitHub, GitLab, Bitbucket

#### Request Changes
Mesrai requests changes when finding critical issues.
...
- Mesrai Playbook: Custom rule enforcement
