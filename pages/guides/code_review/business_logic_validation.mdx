...
description: "Compare your pull request against business rules or specs with @mesrai -v business-logic."
---
...
Business Logic Validation lets you confirm that a pull request implements the expected behaviour described in a spec, document, or ticket. Mesrai analyzes the PR diff, brings in the referenced business context, and flags mismatches before you merge.

## Prerequisites
- If you want Mesrai to **automatically fetch task context** from tools like Jira, Linear, Notion, or ClickUp, the corresponding plugin must be connected in your workspace's [Plugins](/guides/code_review/plugins) page.
...
When enabled, Mesrai runs business logic validation automatically during every PR review, in addition to the on-demand command.
...
2. Mention Mesrai and add the validation command: `@mesrai -v business-logic ...`.
3. Provide the spec content inline or paste a link that Mesrai can fetch via the available plugins.
4. Submit the comment and wait for Mesrai's response in the same thread.

The command only triggers from the main conversation box; replies inside inline suggestions are ignored.

You can re-run the validation at any time — simply post the command again in a new comment. Follow-ups must be a fresh command; replying directly to Mesrai's message in the Git UI is not supported.

### Examples

- Jira ticket: `@mesrai -v business-logic https://your-org.atlassian.net/jira/software/c/projects/KC/boards/2?selectedIssue=KC-1292`
- Linear issue: `@mesrai -v business-logic https://linear.app/your-team/issue/TEAM-123`
- Notion page: `@mesrai -v business-logic https://www.notion.so/your-workspace/Feature-Spec-abc123`
- Google Doc: `@mesrai -v business-logic https://docs.google.com/document/d/1234567890/edit`
- Slack conversation: `@mesrai -v business-logic https://your-org.slack.com/archives/C070E5E97DE/p1727814000000000`
- Inline spec snippet: `@mesrai -v business-logic Rule XYZ — orders above $500 must issue cashback credits.`
...
If Mesrai cannot reach the resource or lacks permissions, it will call that out so you can adjust access or provide the information another way.

## Task Context Quality

Mesrai automatically classifies the quality of the task context before analysis, which affects the depth of validation:
...
Mesrai does not invent requirements. If a finding cannot be traced back to a specific sentence in the task, it is not reported.

### Scope Mismatch Detection

If the PR diff appears to be working in a different domain than the task itself, Mesrai detects this as a **scope mismatch** and reports it as the leading finding, rather than producing misleading gap analysis.
...
**Task:** KC-1441 - Team-scoped Mesrai rules
**Task Link:** https://your-org.atlassian.net/browse/KC-1441
**Status:** Issues Found
**Confidence:** high
...
---
*Analysis performed by Mesrai AI Business Rules Validator*
```
