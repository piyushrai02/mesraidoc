...
description: "Create a Bitbucket API token for Mesrai code review access."
---
...
To enable Mesrai to perform automatic code reviews on Bitbucket Cloud, you need to generate an **API Token**. This token allows Mesrai to access your repositories, pull requests, webhooks, issues, pipelines etc., with the specific permissions listed below.
...
5. Give the token a name (for example `mesrai_code_review`) and set an expiration date (if applicable).
...
## Required Scopes for Mesrai

The token must include **all** of the following scopes for full functionality with Mesrai:
...
- Once the token is generated with all the required scopes, paste it into the **Mesrai integration / automation setup screen** when enabling the Bitbucket integration.
- Make sure the token has `admin:repository:bitbucket` (or equivalent) if Mesrai needs to configure webhooks or listen for pull request events.
