---
title: "AI Agents"
description: "Integrate the Mesrai CLI with AI coding agents for autonomous review-fix loops."
---
...
For any other agent that can run shell commands and parse structured text, `mesrai review --prompt-only` works out of the box.
...
1. Installs the `@mesrai/cli` binary globally.
2. Configures the team key in `~/.mesrai/config.json`.
3. Deploys Mesrai skills to every detected agent (Claude Code, Cursor, Codex, Windsurf).
...
<Steps>
  ...
  <Step title="Agent re-runs review">
    ```bash
    mesrai review --prompt-only --fail-on error
    ```
  ...
  <Step title="Agent commits and pushes">
    Once the review is clean, the agent commits and pushes. Optionally, install the pre-push hook (`mesrai hook install`) for an additional safety net on manual pushes.
  </Step>
</Steps>
...
Generate a key at <a href="https://app.mesrai.com/organization/cli-keys" target="_blank">app.mesrai.com/organization/cli-keys</a> and distribute the install command shown there to your team.
...
# Now every agent turn writes a decision file under .mesrai/
```
...
mesrai review --prompt-only --fail-on error    # general review
mesrai pr business-validation --task-id KC-1441 --dry-run   # check against task
mesrai review --prompt-only --fix               # auto-apply fixable issues
git commit -am "fix: KC-1441 address review findings"
```

With the skill installer, Claude Code's Mesrai skill knows when to invoke each of these — no prompt engineering required on your side.
