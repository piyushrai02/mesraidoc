...
<Callout type="info">
**Smart Token Management**: Mesrai optimizes context to fit within token limits while maintaining review quality. We prioritize the most relevant code sections.
</Callout>
...
**Performance Tip**: For large PRs (>50 files), enable incremental review mode to focus on critical changes first.
</Callout>
...
const analytics = await mesrai.getAnalytics({
  repo: 'my-org/my-repo',
  dateRange: 'last_30_days',
  metrics: ['coverage', 'issues', 'velocity']
})
```
...
### Rich API
Integrate Mesrai into your workflow:
...
### Custom Rules
Define your own review criteria:
...
rules:
  ...
  - name: "Required tests for services"
    require: "*.test.js"
    for: "services/**/*.js"
```
