...
We analyze code changes through multiple dependency layers:
...
<Callout type="info">
Configure review depth via the [Mesrai Dashboard](https://app.mesrai.com) under Settings → Review Depth.
</Callout>
...
### Manual Triggers
You can also request reviews manually via GitHub comments:
...
Configure when reviews should run via the [Dashboard](https://app.mesrai.com) under Settings → Review Triggers:

- **Branch filters**: Always review main, production, develop
- **Path filters**: Only review when specific files change (e.g., `src/**/*.ts`)
- **Ignore patterns**: Skip test files (e.g., `**/*.test.ts`)
- **Skip users**: Exclude bot accounts (dependabot, renovate)
...
### Custom Training
Enterprise customers can work with our team to fine-tune models on their codebase. Contact sales@mesrai.com for more information.
...
Configure review behavior via the [Mesrai Dashboard](https://app.mesrai.com):

### Configure Review Behavior

Navigate to Settings → Review Settings:

- **Review depth**: Quick, Standard, Deep, or Custom
- **Checks enabled**: Bugs, Security, Performance, Style, Tests
- **Severity threshold**: Filter to show only Medium+ issues
- **Auto-approve**: Enable for safe PRs (small files, no critical issues, has tests)

### Team Preferences

Navigate to Settings → Team Preferences:

- **Style guide**: Choose Airbnb, Google, or Standard
- **Language rules**: TypeScript strict mode, no-any warnings
- **Custom rules**: Add patterns like "No TODO in production" for specific branches
