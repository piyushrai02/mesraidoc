...
description: "Solve common issues with Mesrai."
icon: "headset"
---
...
```bash
docker-compose ps mesrai-web
```

2. Verify the ports are correctly mapped:

```bash
docker-compose port mesrai-web 3000
```

3. Check the web container logs:

```bash
docker-compose logs mesrai-web
```

### "Edge Runtime errors in `mesrai-web` logs"
...
1. Update to the latest `mesrai-web` image (re-run the installer or run `docker-compose pull mesrai-web` and `docker-compose up -d`).
...
```bash
docker-compose ps db_mesrai_postgres
docker-compose ps db_mesrai_mongodb
```

2. Verify database logs:

```bash
docker-compose logs db_mesrai_postgres
docker-compose logs db_mesrai_mongodb
```

3. Test database connections:

```bash
docker-compose exec db_mesrai_postgres psql -U $API_PG_DB_USERNAME -d $API_PG_DB_DATABASE
docker-compose exec db_mesrai_mongodb mongosh -u $API_MG_DB_USERNAME -p $API_MG_DB_PASSWORD
```
...
- [Discord Community](mailto:info@mesrai.com)
- [GitHub Issues](https://github.com/mesraiofficial/mesrai-installer/issues)
