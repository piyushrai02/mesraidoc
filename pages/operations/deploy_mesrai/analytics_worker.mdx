...
description: "Optional analytics ingestion worker for self-hosted Enterprise. Powers the Pulse dashboards (DORA, lifecycle, classifier)."
icon: "chart-line"
tag: "enterprise"
---
...
ingestion cron that powers the Pulse dashboards (DORA-style metrics,
...
license and want the Pulse reports.
</Warning>

## What it does

A separate Node process running the **same image as `worker`** (`mesrai-ai-worker`),
...
```yaml
worker-analytics:
    image: ghcr.io/mesraiofficial/mesrai-ai-worker:latest
    platform: linux/amd64
    container_name: mesrai-worker-analytics
    environment:
        - ENV=production
        - NODE_ENV=production
...
ANALYTICS_PG_DB_DATABASE=mesrai_analytics
ANALYTICS_PG_DB_SCHEMA=analytics
